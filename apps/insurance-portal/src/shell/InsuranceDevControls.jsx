// Chrome-less variant of DevControls for embedded use.
//
// mission-control mounts this inside its own consolidated DevPanel
// when CoPilot is open on an insurance opportunity, so this component
// does NOT render the dark sidebar chrome (no <DevPanel> wrapper).
// The embedder is responsible for the surrounding chrome (Section
// styling matches mission-control's dark DevPanel because we render
// the same Section / Segmented primitives from src/shared/DevPanel.jsx).
//
// What's kept vs. dropped relative to the standalone DevControls:
//   - DROP "View" segmented (no view switcher — mission-control owns
//     the view; embedded AgentView is always the agent surface).
//   - DROP "Workflow state" JsonPeek + reset — the embedder owns
//     workflow state and exposes its own debug surface.
//   - KEEP "Default flow path", "Next verification outcome" (when
//     applicable), "Next quote outcome" — these set dev knobs that
//     drive AgentView lead-origination + webhook outcomes.
//   - KEEP "Agent flow simulators" — these directly drive workflow
//     state via updateWorkflow / EI mock simulateWebhook and are
//     essential for demoing the embedded AgentView end-to-end. NOTE:
//     Wave 14-fu shipped a production "Force status" picker on
//     AgentView (AgentForceStatusBar) that handles plain status
//     advancement — see the comment block above the AgentSimulators
//     component for which simulators were kept (and why) vs. now
//     covered by the picker.
//
// EI contract toggles (Wave 31b-fu):
...
//   quoteSavingsOutcome: 'savings' | 'no_savings'
...
//
// Agent flow simulators (Wave 31b-fu): each step is now triggered
// manually so the agent can pause at each card state:
...
//
// Prop signature:
//   dev:            { flowPath, nextVerificationOutcome, nextQuoteOutcome, quoteSavingsOutcome }
//   updateDev:      (patch) => void
//   workflow:       insurance-portal workflow state (parent-owned)
//   updateWorkflow: (patch) => void
//
// Standalone DevControls remains unchanged for the insurance-portal
// dev shell.
import { Check, ExternalLink, Eye, ShieldAlert } from 'lucide-react';
...
export function InsuranceDevControls({
  dev, updateDev,
