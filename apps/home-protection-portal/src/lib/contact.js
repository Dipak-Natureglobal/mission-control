...
//
// `phone` runs through toNationalPhoneDigits defensively: this file's
// contract is a flat `member.phone`, but if a caller ever hands in a
// member sourced from the canonical `.phones[]` E.164 shape (the same
// trap ADR 30's phone-display bug documents), this keeps
// formatPhoneDisplay(contact.phone) downstream (Confirm.jsx,
// BillingPayment.jsx) rendering correctly instead of a truncated number.
export function seedActiveContact(member) {
  ...
  return {
    contact: {
      ...
      email: member.email || '',
      phone: toNationalPhoneDigits(member.phone) || '',
